...
func TestOverheatingTriggersSickMoodAndDebuff(t *testing.T) {
	prev := NewPet("Linus")
	...
	if !hasDebuff(next.Debuffs, DebuffOverheating) {
		t.Errorf("expected debuff %s, got %v", DebuffOverheating, next.Debuffs)
	}
	if next.Mood != MoodSick {
		t.Errorf("expected mood SICK due to overheating, got %s", next.Mood)
	}
	if next.HP != 99 {
		t.Errorf("expected HP 99 after 1 overheating tick, got %d", next.HP)
	}
}

func TestOverheatingIgnoredWhenSensorsUnavailable(t *testing.T) {
	prev := NewPet("Linus")
	snap := collector.SystemSnapshot{
		CPUPercent:       40,
	...
	if hasDebuff(next.Debuffs, DebuffOverheating) {
		t.Errorf("did not expect debuff %s when sensors are unavailable, got %v", DebuffOverheating, next.Debuffs)
	}
	if next.HP != prev.HP+0 {
		t.Errorf("expected HP to remain %d, got %d", prev.HP, next.HP)
	}
}
